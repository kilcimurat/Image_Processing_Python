...
class ImageProcessing:
    ...
    def reflect_image_process(self, reflect_type):
        if reflect_type == "horizontal":
            ...
            self.image = self.image[::-1, :]
        elif reflect_type == "crossed":
            self.image = self.image[::-1, ::-1]
        return self.image

    def resize_image_process(self, row, column, graphics_view):
        ...
        q_image = QImage(self.image, column, row, column * 3, QImage.Format_RGB888)
    ...
    def show_image_new_window(self, graphics_view):
